...
def main():
    ...
    # ⭐ ВЫХОДИМ ИЗ ЦИКЛА ВЫБОРА ФУНКЦИИ (через break)
    ...
    while True:
        print("\n--- ШАГ 1: ВЫБОР УЗЛОВ ---")
        print("Выберите способ генерации узлов:")
        ...
        try:
            choice_nodes = int(input("Ваш выбор: "))
        except ValueError:
            print("Введите целое число")
            continue
        ...
        if choice_nodes not in [1, 2, 3]:
            print("Введите 1, 2, 3 или 0")
            continue
        
        x_nodes = generate_nodes(choice_nodes, a, b, n)
        y_nodes = f(x_nodes)
        ...
        while True:
            ...
            if choice_method == 0:
                print("\n↩️ Возврат к выбору узлов...")
                break  # ← ВЫХОД ИЗ ЦИКЛА МЕТОДОВ → ВОЗВРАТ К ВЫБОРУ УЗЛОВ
        ...
        # КОНЕЦ ЦИКЛА 3 (после break возвращаемся к выбору узлов)
    
    # КОНЕЦ ЦИКЛА 2 (после continue возвращаемся к выбору функции)
